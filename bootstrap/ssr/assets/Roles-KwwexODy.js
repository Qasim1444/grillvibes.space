...
var _sfc_main = /*@__PURE__*/ Object.assign({ layout: AdminLayout_default }, {
	...
	setup(__props) {
		...
		const grouped = computed(() => props.permissions.reduce((acc, p) => {
			(acc[p.group] ??= []).push(p);
			return acc;
		}, {}));
		...
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<div${ssrRenderAttrs(mergeProps({ class: "page" }, _attrs))} data-v-ab8ec34d>`);
			_push(ssrRenderComponent(_sfc_main$1, {
				...
				actions: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						if (unref(can)("roles.create")) _push(`<button class="ui-btn ui-btn--primary" data-v-ab8ec34d${_scopeId}>+ Add Role</button>`);
						else _push(`<!---->`);
			...
			_push(ssrRenderComponent(DataTable_default, {
				...
				"cell:permissions_count": withCtx(({ row }, _push, _parent, _scopeId) => {
					if (_push) _push(`<span class="${ssrRenderClass([row.is_locked ? "ui-badge--success" : "ui-badge--muted", "ui-badge"])}" data-v-ab8ec34d${_scopeId}>${ssrInterpolate(row.is_locked ? "All permissions" : `${row.permissions_count} permissions`)}</span>`);
				...
				actions: withCtx(({ row }, _push, _parent, _scopeId) => {
					if (_push) {
						if (unref(can)("roles.update")) _push(`<button class="ui-btn ui-btn--ghost ui-btn--sm"${ssrIncludeBooleanAttr(row.is_locked) ? " disabled" : ""}${ssrRenderAttr("title", row.is_locked ? "The Super Admin role is locked." : "")} data-v-ab8ec34d${_scopeId}> Edit </button>`);
						else _push(`<!---->`);
						if (unref(can)("roles.delete")) _push(`<button class="ui-btn ui-btn--danger ui-btn--sm"${ssrIncludeBooleanAttr(row.is_locked) ? " disabled" : ""} data-v-ab8ec34d${_scopeId}> Delete </button>`);
						else _push(`<!---->`);
			...
			_push(ssrRenderComponent(Modal_default, {
				...
				footer: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) _push(`<button class="ui-btn ui-btn--ghost" data-v-ab8ec34d${_scopeId}>Cancel</button><button class="ui-btn ui-btn--primary"${ssrIncludeBooleanAttr(unref(form).processing) ? " disabled" : ""} data-v-ab8ec34d${_scopeId}>Save</button>`);
				...
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						...
						_push(`<div class="ui-field" data-v-ab8ec34d${_scopeId}><div class="roles__matrix-head" data-v-ab8ec34d${_scopeId}><label class="ui-label" data-v-ab8ec34d${_scopeId}>Permissions</label><span class="roles__count" data-v-ab8ec34d${_scopeId}>${ssrInterpolate(unref(form).permission_ids.length)} of ${ssrInterpolate(props.permissions.length)} selected</span></div>`);
						if (unref(form).errors.permission_ids) _push(`<p class="ui-field__error" data-v-ab8ec34d${_scopeId}>${ssrInterpolate(unref(form).errors.permission_ids)}</p>`);
						else _push(`<!---->`);
						_push(`<!--[-->`);
						ssrRenderList(grouped.value, (items, group) => {
							_push(`<div class="roles__group" data-v-ab8ec34d${_scopeId}><label class="roles__group-head" data-v-ab8ec34d${_scopeId}><input type="checkbox"${ssrIncludeBooleanAttr(isGroupFull(items)) ? " checked" : ""}${ssrRenderAttr("indeterminate", isGroupPartial(items))} data-v-ab8ec34d${_scopeId}><span data-v-ab8ec34d${_scopeId}>${ssrInterpolate(group)}</span></label><div class="roles__perms" data-v-ab8ec34d${_scopeId}><!--[-->`);
							ssrRenderList(items, (p) => {
								_push(`<label class="roles__perm" data-v-ab8ec34d${_scopeId}><input type="checkbox"${ssrRenderAttr("value", p.id)}${ssrIncludeBooleanAttr(Array.isArray(unref(form).permission_ids) ? ssrLooseContain(unref(form).permission_ids, p.id) : unref(form).permission_ids) ? " checked" : ""} data-v-ab8ec34d${_scopeId}><span data-v-ab8ec34d${_scopeId}>${ssrInterpolate(p.name)}</span><code data-v-ab8ec34d${_scopeId}>${ssrInterpolate(p.key)}</code></label>`);
							});
							_push(`<!--]--></div></div>`);
						});
						_push(`<!--]--></div>`);
						...
						createVNode("div", { class: "ui-field" }, [
							createVNode("div", { class: "roles__matrix-head" }, [createVNode("label", { class: "ui-label" }, "Permissions"), createVNode("span", { class: "roles__count" }, toDisplayString(unref(form).permission_ids.length) + " of " + toDisplayString(props.permissions.length) + " selected", 1)]),
...
var Roles_default = /*#__PURE__*/ _plugin_vue_export_helper_default(_sfc_main, [["__scopeId", "data-v-ab8ec34d"]]);
//#endregion
