...
var _sfc_main = /*@__PURE__*/ Object.assign({ layout: AdminLayout_default }, {
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<div${ssrRenderAttrs(mergeProps({ class: "page" }, _attrs))} data-v-6546aac5>`);
			_push(ssrRenderComponent(_sfc_main$1, {
				...
				actions: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						if (unref(can)("hr.attendance.create")) _push(`<button class="ui-btn ui-btn--primary"${ssrIncludeBooleanAttr(saving.value || !sheet.value.length) ? " disabled" : ""} data-v-6546aac5${_scopeId}>${ssrInterpolate(saving.value ? "Saving…" : "Save Sheet")}</button>`);
						else _push(`<!---->`);
			...
			_push(`<div class="att__bar ui-card ui-card-pad" data-v-6546aac5><div class="att__date" data-v-6546aac5><label class="ui-label" for="att-date" data-v-6546aac5>Date</label><input id="att-date" class="ui-input" type="date"${ssrRenderAttr("value", date.value)} data-v-6546aac5></div>`);
			if (unref(can)("hr.attendance.create")) _push(`<div class="att__bulk" data-v-6546aac5><span class="ui-label" data-v-6546aac5>Mark everyone</span><div class="att__bulk-btns" data-v-6546aac5><button class="ui-btn ui-btn--ghost ui-btn--sm" data-v-6546aac5>Present</button><button class="ui-btn ui-btn--ghost ui-btn--sm" data-v-6546aac5>Absent</button><button class="ui-btn ui-btn--ghost ui-btn--sm" data-v-6546aac5>Holiday</button></div></div>`);
			else _push(`<!---->`);
			_push(`<div class="att__tallies" data-v-6546aac5><!--[-->`);
			ssrRenderList(props.statuses, (s) => {
				_push(`<span class="${ssrRenderClass([badgeClass(s), "ui-badge"])}" data-v-6546aac5>${ssrInterpolate(label(s))}: ${ssrInterpolate(tallies.value[s] || 0)}</span>`);
			});
			_push(`<!--]--><span class="ui-badge ui-badge--info" data-v-6546aac5>Total: ${ssrInterpolate(sheet.value.length)}</span></div></div>`);
			if (dirty.value) _push(`<div class="ui-alert ui-alert--danger" data-v-6546aac5> You have unsaved changes on this sheet — click <strong data-v-6546aac5>Save Sheet</strong> before switching dates. </div>`);
			else _push(`<!---->`);
			if (unref(can)("branches.update")) {
				_push(`<div class="att__settings ui-card ui-card-pad" data-v-6546aac5><div data-v-6546aac5><strong data-v-6546aac5>Branch attendance locations</strong><p class="att__hint" data-v-6546aac5>Each employee is checked against the branch assigned to their place.</p></div><!--[-->`);
				ssrRenderList(branchForms.value, (branch) => {
					_push(`<div class="att__settings-row" data-v-6546aac5><strong data-v-6546aac5>${ssrInterpolate(branch.name)}</strong><input${ssrRenderAttr("value", branch.latitude)} class="ui-input" type="number" step="0.0000001" min="-90" max="90" placeholder="Latitude" data-v-6546aac5><input${ssrRenderAttr("value", branch.longitude)} class="ui-input" type="number" step="0.0000001" min="-180" max="180" placeholder="Longitude" data-v-6546aac5><input${ssrRenderAttr("value", branch.attendance_radius_meters)} class="ui-input att__num" type="number" min="10" max="5000" title="Allowed radius in meters" data-v-6546aac5><input${ssrRenderAttr("value", branch.attendance_start_time)} class="ui-input" type="time" title="Shift start time" data-v-6546aac5><button class="ui-btn ui-btn--ghost ui-btn--sm"${ssrIncludeBooleanAttr(branch.saving) ? " disabled" : ""} data-v-6546aac5>${ssrInterpolate(branch.saving ? "Saving…" : "Save")}</button></div>`);
				});
				_push(`<!--]-->`);
				if (!branchForms.value.length) _push(`<p class="att__hint" data-v-6546aac5>No branches have been configured.</p>`);
				else _push(`<!---->`);
				_push(`</div>`);
			} else _push(`<!---->`);
			_push(`<div class="stat-grid" data-v-6546aac5>`);
			...
			_push(ssrRenderComponent(DataTable_default, {
				...
				"cell:name": withCtx(({ row }, _push, _parent, _scopeId) => {
					if (_push) {
						_push(`<div class="att__name" data-v-6546aac5${_scopeId}><strong data-v-6546aac5${_scopeId}>${ssrInterpolate(row.name)}</strong>`);
						if (row.employee_code) _push(`<span class="att__code" data-v-6546aac5${_scopeId}>${ssrInterpolate(row.employee_code)}</span>`);
						else _push(`<!---->`);
						_push(`</div>`);
				...
				"cell:status": withCtx(({ row }, _push, _parent, _scopeId) => {
					if (_push) {
						_push(`<select class="ui-select att__select"${ssrIncludeBooleanAttr(!editable.value) ? " disabled" : ""} data-v-6546aac5${_scopeId}><!--[-->`);
						ssrRenderList(props.statuses, (s) => {
							_push(`<option${ssrRenderAttr("value", s)} data-v-6546aac5${ssrIncludeBooleanAttr(Array.isArray(row.status) ? ssrLooseContain(row.status, s) : ssrLooseEqual(row.status, s)) ? " selected" : ""}${_scopeId}>${ssrInterpolate(label(s))}</option>`);
						});
						_push(`<!--]--></select>`);
				...
				"cell:check_in": withCtx(({ row }, _push, _parent, _scopeId) => {
					if (_push) {
						_push(`<div class="att__event" data-v-6546aac5${_scopeId}><input${ssrRenderAttr("value", row.check_in)} class="ui-input att__time" type="time"${ssrIncludeBooleanAttr(!editable.value) ? " disabled" : ""} data-v-6546aac5${_scopeId}>`);
						if (row.check_in_distance_meters !== null) _push(`<small class="att__verified" data-v-6546aac5${_scopeId}>GPS ${ssrInterpolate(row.check_in_distance_meters)}m</small>`);
						else _push(`<!---->`);
						_push(`</div>`);
				...
				"cell:check_out": withCtx(({ row }, _push, _parent, _scopeId) => {
					if (_push) {
						_push(`<div class="att__event" data-v-6546aac5${_scopeId}><input${ssrRenderAttr("value", row.check_out)} class="ui-input att__time" type="time"${ssrIncludeBooleanAttr(!editable.value) ? " disabled" : ""} data-v-6546aac5${_scopeId}>`);
						if (row.check_out_distance_meters !== null) _push(`<small class="att__verified" data-v-6546aac5${_scopeId}>GPS ${ssrInterpolate(row.check_out_distance_meters)}m</small>`);
						else _push(`<!---->`);
						_push(`</div>`);
					} else return [createVNode("div", { class: "att__event" }, [withDirectives(createVNode("input", {
				...
				"cell:late_minutes": withCtx(({ row }, _push, _parent, _scopeId) => {
					if (_push) _push(`<input${ssrRenderAttr("value", row.late_minutes)} class="ui-input att__num" type="number" min="0" max="1440"${ssrIncludeBooleanAttr(!editable.value) ? " disabled" : ""} data-v-6546aac5${_scopeId}>`);
				...
				"cell:note": withCtx(({ row }, _push, _parent, _scopeId) => {
					if (_push) _push(`<input${ssrRenderAttr("value", row.note)} class="ui-input" type="text" placeholder="Note"${ssrIncludeBooleanAttr(!editable.value) ? " disabled" : ""} data-v-6546aac5${_scopeId}>`);
					else return [withDirectives(createVNode("input", {
				...
				actions: withCtx(({ row }, _push, _parent, _scopeId) => {
					if (_push) {
						if (row.attendance_id && unref(can)("hr.attendance.delete")) _push(`<button class="ui-btn ui-btn--danger ui-btn--sm" data-v-6546aac5${_scopeId}> Clear </button>`);
						else _push(`<span class="att__unsaved" data-v-6546aac5${_scopeId}>${ssrInterpolate(row.attendance_id ? "" : "Not saved")}</span>`);
...
var Attendance_default = /*#__PURE__*/ _plugin_vue_export_helper_default(_sfc_main, [["__scopeId", "data-v-6546aac5"]]);
//#endregion
