...
var _sfc_main = /*@__PURE__*/ Object.assign({ layout: AdminLayout_default }, {
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<div${ssrRenderAttrs(mergeProps({ class: "page" }, _attrs))} data-v-9aa3599b>`);
			...
			_push(`<div class="rc__kpis" data-v-9aa3599b><div class="rc__kpi rc__kpi--info" data-v-9aa3599b><span class="rc__kpi-label" data-v-9aa3599b>Menu</span><span class="rc__kpi-val" data-v-9aa3599b>${ssrInterpolate(props.summary.total_dishes)}</span><span class="rc__kpi-sub" data-v-9aa3599b>${ssrInterpolate(props.summary.costed)} with a recipe</span></div><div class="${ssrRenderClass([props.summary.uncosted > 0 ? "rc__kpi--warn" : "rc__kpi--ok", "rc__kpi"])}" data-v-9aa3599b><span class="rc__kpi-label" data-v-9aa3599b>Uncosted</span><span class="rc__kpi-val" data-v-9aa3599b>${ssrInterpolate(props.summary.uncosted)}</span><span class="rc__kpi-sub" data-v-9aa3599b>no recipe, so no COGS</span></div><div class="rc__kpi rc__kpi--ok" data-v-9aa3599b><span class="rc__kpi-label" data-v-9aa3599b>Avg. Food Cost</span><span class="rc__kpi-val" data-v-9aa3599b>${ssrInterpolate(props.summary.avg_food_cost === null ? "—" : props.summary.avg_food_cost + "%")}</span><span class="rc__kpi-sub" data-v-9aa3599b>across costed dishes</span></div><div class="${ssrRenderClass([props.summary.over_target > 0 ? "rc__kpi--danger" : "rc__kpi--ok", "rc__kpi"])}" data-v-9aa3599b><span class="rc__kpi-label" data-v-9aa3599b>Over 40%</span><span class="rc__kpi-val" data-v-9aa3599b>${ssrInterpolate(props.summary.over_target)}</span><span class="rc__kpi-sub" data-v-9aa3599b>where margin leaks</span></div></div>`);
			if (missingDeepLink.value) _push(`<p class="rc__notice" data-v-9aa3599b> That dish isn&#39;t on this page — search for it by name to open its recipe. </p>`);
			else _push(`<!---->`);
			_push(`<div class="rc__filters" data-v-9aa3599b><input${ssrRenderAttr("value", search.value)} class="ui-input rc__search" placeholder="Search dish / code…" data-v-9aa3599b><select class="ui-input" style="${ssrRenderStyle({ "width": "200px" })}" data-v-9aa3599b><option value="" data-v-9aa3599b${ssrIncludeBooleanAttr(Array.isArray(categoryFilter.value) ? ssrLooseContain(categoryFilter.value, "") : ssrLooseEqual(categoryFilter.value, "")) ? " selected" : ""}>All Categories</option><!--[-->`);
			ssrRenderList(props.categories, (c) => {
				_push(`<option${ssrRenderAttr("value", c.id)} data-v-9aa3599b${ssrIncludeBooleanAttr(Array.isArray(categoryFilter.value) ? ssrLooseContain(categoryFilter.value, c.id) : ssrLooseEqual(categoryFilter.value, c.id)) ? " selected" : ""}>${ssrInterpolate(c.name)}</option>`);
			});
			_push(`<!--]--></select><div class="rc__tabs" data-v-9aa3599b><!--[-->`);
			ssrRenderList(tabs, (tab) => {
				_push(`<button class="${ssrRenderClass([{ "rc__tab--on": viewFilter.value === tab.value }, "rc__tab"])}" data-v-9aa3599b>${ssrInterpolate(tab.label)}</button>`);
			});
			_push(`<!--]--></div></div>`);
			_push(ssrRenderComponent(DataTable_default, {
				...
				"cell:name": withCtx(({ row }, _push, _parent, _scopeId) => {
					if (_push) {
						_push(`<strong data-v-9aa3599b${_scopeId}>${ssrInterpolate(row.name)}</strong>`);
						if (row.code) _push(`<span class="rc__muted rc__code" data-v-9aa3599b${_scopeId}>${ssrInterpolate(row.code)}</span>`);
						else _push(`<!---->`);
						_push(`<span class="rc__cat" data-v-9aa3599b${_scopeId}>${ssrInterpolate(row.category_name || "Uncategorised")}</span>`);
					} else return [
				...
				"cell:dish_cost": withCtx(({ row }, _push, _parent, _scopeId) => {
					if (_push) {
						if (row.line_count === 0) _push(`<span class="ui-badge ui-badge--muted" data-v-9aa3599b${_scopeId}>No recipe</span>`);
						else _push(`<!--[-->${ssrInterpolate(fmt(row.dish_cost))} <span class="rc__muted rc__lines" data-v-9aa3599b${_scopeId}>${ssrInterpolate(row.line_count)} ingredient(s)</span><!--]-->`);
				...
				"cell:food_cost_percent": withCtx(({ row }, _push, _parent, _scopeId) => {
					if (_push) {
						if (row.food_cost_percent === null) _push(`<span class="rc__muted" data-v-9aa3599b${_scopeId}>—</span>`);
						else _push(`<span class="${ssrRenderClass([pctBadge(row.food_cost_percent), "ui-badge"])}" data-v-9aa3599b${_scopeId}>${ssrInterpolate(row.food_cost_percent)}%</span>`);
				...
				"cell:margin": withCtx(({ row }, _push, _parent, _scopeId) => {
					if (_push) {
						if (row.margin === null) _push(`<span class="rc__muted" data-v-9aa3599b${_scopeId}>—</span>`);
						else _push(`<span class="${ssrRenderClass(row.margin < 0 ? "rc__neg" : "")}" data-v-9aa3599b${_scopeId}>${ssrInterpolate(fmt(row.margin))}</span>`);
				...
				"cell:portions_available": withCtx(({ row }, _push, _parent, _scopeId) => {
					if (_push) {
						if (row.portions_available === null) _push(`<span class="rc__muted" data-v-9aa3599b${_scopeId}>—</span>`);
						else _push(`<span class="${ssrRenderClass(row.portions_available === 0 ? "rc__neg" : row.portions_available < 10 ? "rc__low" : "")}" data-v-9aa3599b${_scopeId}>${ssrInterpolate(row.portions_available)}</span>`);
				...
				actions: withCtx(({ row }, _push, _parent, _scopeId) => {
					if (_push) _push(`<button class="ui-btn ui-btn--ghost ui-btn--sm" data-v-9aa3599b${_scopeId}>${ssrInterpolate(unref(can)("inventory.recipes.update") ? row.line_count ? "Edit Recipe" : "Add Recipe" : "View Recipe")}</button>`);
			...
			_push(ssrRenderComponent(Modal_default, {
				...
				footer: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						_push(`<button class="ui-btn ui-btn--ghost" data-v-9aa3599b${_scopeId}>${ssrInterpolate(editable.value ? "Cancel" : "Close")}</button>`);
						if (editable.value) _push(`<button class="ui-btn ui-btn--primary"${ssrIncludeBooleanAttr(unref(form).processing || hasDuplicates.value) ? " disabled" : ""} data-v-9aa3599b${_scopeId}> Save Recipe </button>`);
						else _push(`<!---->`);
					} else return [createVNode("button", {
				...
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						if (dish.value) {
							_push(`<div class="rc__build" data-v-9aa3599b${_scopeId}><div class="rc__live" data-v-9aa3599b${_scopeId}><div class="rc__live-cell" data-v-9aa3599b${_scopeId}><span class="rc__live-label" data-v-9aa3599b${_scopeId}>Selling Price</span><span class="rc__live-val" data-v-9aa3599b${_scopeId}>${ssrInterpolate(fmt(dish.value.price))}</span></div><div class="rc__live-cell" data-v-9aa3599b${_scopeId}><span class="rc__live-label" data-v-9aa3599b${_scopeId}>Dish Cost</span><span class="rc__live-val" data-v-9aa3599b${_scopeId}>${ssrInterpolate(fmt(liveCost.value))}</span></div><div class="rc__live-cell" data-v-9aa3599b${_scopeId}><span class="rc__live-label" data-v-9aa3599b${_scopeId}>Food Cost</span><span class="${ssrRenderClass([livePctClass.value, "rc__live-val"])}" data-v-9aa3599b${_scopeId}>${ssrInterpolate(livePercent.value === null ? "—" : livePercent.value + "%")}</span></div><div class="rc__live-cell" data-v-9aa3599b${_scopeId}><span class="rc__live-label" data-v-9aa3599b${_scopeId}>Margin</span><span class="${ssrRenderClass([liveMargin.value < 0 ? "rc__neg" : "rc__pos", "rc__live-val"])}" data-v-9aa3599b${_scopeId}>${ssrInterpolate(fmt(liveMargin.value))}</span></div></div><p class="rc__hint" data-v-9aa3599b${_scopeId}> Quantities are per portion, in each ingredient&#39;s own unit. Unit costs are this outlet&#39;s moving average — an ingredient with no delivery booked yet costs nothing, which will understate the dish until it is received. </p><table class="rc__table" data-v-9aa3599b${_scopeId}><thead data-v-9aa3599b${_scopeId}><tr data-v-9aa3599b${_scopeId}><th style="${ssrRenderStyle({ "width": "34%" })}" data-v-9aa3599b${_scopeId}>Ingredient</th><th style="${ssrRenderStyle({ "width": "16%" })}" data-v-9aa3599b${_scopeId}>Qty / Portion</th><th style="${ssrRenderStyle({ "width": "14%" })}" class="rc__right" data-v-9aa3599b${_scopeId}>Unit Cost</th><th style="${ssrRenderStyle({ "width": "14%" })}" class="rc__right" data-v-9aa3599b${_scopeId}>Line Cost</th><th data-v-9aa3599b${_scopeId}>Note</th><th style="${ssrRenderStyle({ "width": "40px" })}" data-v-9aa3599b${_scopeId}></th></tr></thead><tbody data-v-9aa3599b${_scopeId}>`);
							if (!unref(form).lines.length) _push(`<tr data-v-9aa3599b${_scopeId}><td colspan="6" class="rc__muted rc__empty" data-v-9aa3599b${_scopeId}> No ingredients yet. Add the first line to start costing this dish. </td></tr>`);
							else _push(`<!---->`);
							_push(`<!--[-->`);
							ssrRenderList(unref(form).lines, (line, i) => {
								_push(`<tr data-v-9aa3599b${_scopeId}><td data-v-9aa3599b${_scopeId}><select class="ui-input"${ssrIncludeBooleanAttr(!editable.value) ? " disabled" : ""} data-v-9aa3599b${_scopeId}><option value="" data-v-9aa3599b${ssrIncludeBooleanAttr(Array.isArray(line.ingredient_id) ? ssrLooseContain(line.ingredient_id, "") : ssrLooseEqual(line.ingredient_id, "")) ? " selected" : ""}${_scopeId}>Select…</option><!--[-->`);
								ssrRenderList(props.ingredients, (ing) => {
									_push(`<option${ssrRenderAttr("value", ing.id)} data-v-9aa3599b${ssrIncludeBooleanAttr(Array.isArray(line.ingredient_id) ? ssrLooseContain(line.ingredient_id, ing.id) : ssrLooseEqual(line.ingredient_id, ing.id)) ? " selected" : ""}${_scopeId}>${ssrInterpolate(ing.name)}</option>`);
								});
								_push(`<!--]--></select>`);
								if (isDuplicate(line, i)) _push(`<span class="rc__err" data-v-9aa3599b${_scopeId}>Already listed above</span>`);
								else _push(`<!---->`);
								_push(`</td><td data-v-9aa3599b${_scopeId}><div class="rc__qty" data-v-9aa3599b${_scopeId}><input${ssrRenderAttr("value", line.quantity)} type="number" step="0.0001" min="0" class="ui-input"${ssrIncludeBooleanAttr(!editable.value) ? " disabled" : ""} data-v-9aa3599b${_scopeId}><span class="rc__unit" data-v-9aa3599b${_scopeId}>${ssrInterpolate(unitOf(line.ingredient_id))}</span></div></td><td class="rc__right rc__muted" data-v-9aa3599b${_scopeId}>${ssrInterpolate(fmt(costOf(line.ingredient_id)))}</td><td class="rc__right" data-v-9aa3599b${_scopeId}><strong data-v-9aa3599b${_scopeId}>${ssrInterpolate(fmt(lineCost(line)))}</strong></td><td data-v-9aa3599b${_scopeId}><input${ssrRenderAttr("value", line.note)} class="ui-input" placeholder="e.g. diced"${ssrIncludeBooleanAttr(!editable.value) ? " disabled" : ""} data-v-9aa3599b${_scopeId}></td><td data-v-9aa3599b${_scopeId}>`);
								if (editable.value) _push(`<button class="ui-btn ui-btn--danger ui-btn--sm" title="Remove line" data-v-9aa3599b${_scopeId}>×</button>`);
								else _push(`<!---->`);
								_push(`</td></tr>`);
							});
							_push(`<!--]--></tbody>`);
							if (unref(form).lines.length) _push(`<tfoot data-v-9aa3599b${_scopeId}><tr data-v-9aa3599b${_scopeId}><td colspan="3" class="rc__right" data-v-9aa3599b${_scopeId}><strong data-v-9aa3599b${_scopeId}>Total</strong></td><td class="rc__right" data-v-9aa3599b${_scopeId}><strong data-v-9aa3599b${_scopeId}>${ssrInterpolate(fmt(liveCost.value))}</strong></td><td colspan="2" data-v-9aa3599b${_scopeId}></td></tr></tfoot>`);
							else _push(`<!---->`);
							_push(`</table>`);
							if (unref(form).errors.lines) _push(`<p class="rc__err" data-v-9aa3599b${_scopeId}>${ssrInterpolate(unref(form).errors.lines)}</p>`);
							else _push(`<!---->`);
							if (editable.value) _push(`<button class="ui-btn ui-btn--secondary ui-btn--sm rc__add" data-v-9aa3599b${_scopeId}>+ Add Ingredient</button>`);
							else _push(`<!---->`);
							if (dish.value.portions_available !== null) _push(`<p class="rc__stockline" data-v-9aa3599b${_scopeId}> Current stock can make <strong data-v-9aa3599b${_scopeId}>${ssrInterpolate(dish.value.portions_available)}</strong> portion(s) of this dish as saved. </p>`);
							else _push(`<!---->`);
							_push(`</div>`);
						} else _push(`<!---->`);
...
var Recipes_default = /*#__PURE__*/ _plugin_vue_export_helper_default(_sfc_main, [["__scopeId", "data-v-9aa3599b"]]);
//#endregion
