...
var _sfc_main = /*@__PURE__*/ Object.assign({ layout: AdminLayout_default }, {
	...
	setup(__props) {
		...
		const saveItem = () => {
			form.transform((data) => {
				const payload = { ...data };
				if (form.id && !payload.image) delete payload.image;
				return form.id ? {
					...payload,
					_method: "put"
				} : payload;
		...
		const deleteItem = async (id) => {
			if (!await confirmDialog("Are you sure you want to delete this food item?")) return;
		...
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<div${ssrRenderAttrs(mergeProps({ class: "page" }, _attrs))} data-v-59ecb552>`);
			_push(ssrRenderComponent(_sfc_main$1, {
				...
				actions: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						if (unref(can)("food-items.create")) _push(`<button class="ui-btn ui-btn--primary" data-v-59ecb552${_scopeId}> + Add Food Item </button>`);
						else _push(`<!---->`);
			...
			_push(ssrRenderComponent(DataTable_default, {
				...
				"cell:image": withCtx(({ value }, _push, _parent, _scopeId) => {
					if (_push) {
						if (value) _push(`<img${ssrRenderAttr("src", value)} alt="" class="food-item__thumb" data-v-59ecb552${_scopeId}>`);
						else _push(`<span class="food-item__thumb food-item__thumb--empty" data-v-59ecb552${_scopeId}> — </span>`);
				...
				"cell:status": withCtx(({ value }, _push, _parent, _scopeId) => {
					if (_push) _push(`<span class="${ssrRenderClass([value ? "ui-badge--success" : "ui-badge--muted", "ui-badge"])}" data-v-59ecb552${_scopeId}>${ssrInterpolate(value ? "Active" : "Inactive")}</span>`);
				...
				actions: withCtx(({ row }, _push, _parent, _scopeId) => {
					if (_push) {
						...
						if (unref(can)("food-items.update")) _push(`<button class="ui-btn ui-btn--ghost ui-btn--sm" data-v-59ecb552${_scopeId}> Edit </button>`);
						else _push(`<!---->`);
						if (unref(can)("food-items.delete")) _push(`<button class="ui-btn ui-btn--danger ui-btn--sm" data-v-59ecb552${_scopeId}> Delete </button>`);
						else _push(`<!---->`);
					} else return [
			...
			_push(ssrRenderComponent(Modal_default, {
				...
				footer: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) _push(`<button type="button" class="ui-btn ui-btn--ghost" data-v-59ecb552${_scopeId}> Cancel </button><button type="button" class="ui-btn ui-btn--primary"${ssrIncludeBooleanAttr(unref(form).processing) ? " disabled" : ""} data-v-59ecb552${_scopeId}>${ssrInterpolate(unref(form).processing ? "Saving..." : "Save")}</button>`);
				...
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						...
						_push(ssrRenderComponent(_sfc_main$2, {
							...
							default: withCtx((_, _push, _parent, _scopeId) => {
								if (_push) {
									_push(`<option value="" disabled data-v-59ecb552${_scopeId}> Select a category </option><!--[-->`);
									ssrRenderList(categories.value, (cat) => {
										_push(`<option${ssrRenderAttr("value", cat.id)} data-v-59ecb552${_scopeId}>${ssrInterpolate(cat.name)}</option>`);
									});
									_push(`<!--]-->`);
						...
						_push(`<div class="ui-field" data-v-59ecb552${_scopeId}><label class="ui-label" data-v-59ecb552${_scopeId}>Image</label>`);
						if (imagePreview.value) _push(`<img${ssrRenderAttr("src", imagePreview.value)} alt="" class="food-item__preview" data-v-59ecb552${_scopeId}>`);
						else _push(`<!---->`);
						_push(`<input type="file" class="ui-input" accept="image/*" data-v-59ecb552${_scopeId}>`);
						if (unref(form).errors.image) _push(`<span class="ui-field__error" data-v-59ecb552${_scopeId}>${ssrInterpolate(unref(form).errors.image)}</span>`);
						else _push(`<!---->`);
						_push(`</div>`);
						_push(ssrRenderComponent(_sfc_main$2, {
							...
							default: withCtx((_, _push, _parent, _scopeId) => {
								if (_push) _push(`<option${ssrRenderAttr("value", true)} data-v-59ecb552${_scopeId}> Active </option><option${ssrRenderAttr("value", false)} data-v-59ecb552${_scopeId}> Inactive </option>`);
...
var FoodItems_default = /*#__PURE__*/ _plugin_vue_export_helper_default(_sfc_main, [["__scopeId", "data-v-59ecb552"]]);
//#endregion
