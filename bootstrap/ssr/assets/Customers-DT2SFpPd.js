...
import { n as confirmDialog, t as alertDialog } from "./useNotifications-CvzCW6Mx.js";
	...
	setup(__props) {
		...
		*/
		...
		const sendMessage = async () => {
			...
			try {
				...
				if (res.success) {
					...
					clearAttachment();
					await loadMessages();
				} else alertDialog(res.message || "Failed to send message.");
			} catch (err) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<div${ssrRenderAttrs(mergeProps({ class: "page" }, _attrs))} data-v-2cba5b1a>`);
			_push(ssrRenderComponent(_sfc_main$1, {
				...
				actions: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						_push(`<button class="ui-btn ui-btn--ghost" data-v-2cba5b1a${_scopeId}>Message a Number</button>`);
						if (unref(can)("customers.create")) _push(`<button class="ui-btn ui-btn--primary" data-v-2cba5b1a${_scopeId}>+ Add Customer</button>`);
						else _push(`<!---->`);
			...
			_push(ssrRenderComponent(DataTable_default, {
				...
				"cell:name": withCtx(({ row }, _push, _parent, _scopeId) => {
					if (_push) _push(`<button class="cust-name-btn"${ssrRenderAttr("title", `View ${row.name}'s details`)} data-v-2cba5b1a${_scopeId}>${ssrInterpolate(row.name)}</button>`);
					else return [createVNode("button", {
						class: "cust-name-btn",
				...
				actions: withCtx(({ row }, _push, _parent, _scopeId) => {
					if (_push) {
						_push(`<button class="${ssrRenderClass([{ "has-unread": getUnreadCount(row.contact) > 0 }, "ui-btn ui-btn--ghost ui-btn--sm ui-btn-icon"])}" title="Chat on WhatsApp" data-v-2cba5b1a${_scopeId}><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" data-v-2cba5b1a${_scopeId}><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" data-v-2cba5b1a${_scopeId}></path></svg>`);
						if (getUnreadCount(row.contact) > 0) _push(`<span class="unread-badge" data-v-2cba5b1a${_scopeId}>${ssrInterpolate(getUnreadCount(row.contact))}</span>`);
						else _push(`<!---->`);
						_push(`</button><button class="${ssrRenderClass([{ "has-calls": getCallCount(row.contact) > 0 }, "ui-btn ui-btn--ghost ui-btn--sm ui-btn-icon ui-btn-icon--call"])}" title="Call history" data-v-2cba5b1a${_scopeId}><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" data-v-2cba5b1a${_scopeId}><path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z" data-v-2cba5b1a${_scopeId}></path></svg>`);
						if (getCallCount(row.contact) > 0) _push(`<span class="call-badge" data-v-2cba5b1a${_scopeId}>${ssrInterpolate(getCallCount(row.contact))}</span>`);
						else _push(`<!---->`);
						_push(`</button>`);
						if (unref(can)("customers.update")) _push(`<button class="ui-btn ui-btn--ghost ui-btn--sm" data-v-2cba5b1a${_scopeId}>Edit</button>`);
						else _push(`<!---->`);
						if (unref(can)("customers.delete")) _push(`<button class="ui-btn ui-btn--danger ui-btn--sm" data-v-2cba5b1a${_scopeId}>Delete</button>`);
						else _push(`<!---->`);
					} else return [
						createVNode("button", {
			...
			_push(ssrRenderComponent(Modal_default, {
				...
				footer: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) _push(`<button class="ui-btn ui-btn--ghost" data-v-2cba5b1a${_scopeId}>Cancel</button><button class="ui-btn ui-btn--primary"${ssrIncludeBooleanAttr(unref(form).processing) ? " disabled" : ""} data-v-2cba5b1a${_scopeId}>Save</button>`);
					else return [createVNode("button", {
						class: "ui-btn ui-btn--ghost",
			...
			_push(ssrRenderComponent(Modal_default, {
				...
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						_push(`<div class="chat-header" data-v-2cba5b1a${_scopeId}><div class="chat-header__avatar" data-v-2cba5b1a${_scopeId}>${ssrInterpolate((chatCustomer.value?.name || "?").charAt(0).toUpperCase())}</div><div class="chat-header__info" data-v-2cba5b1a${_scopeId}><div class="chat-header__name" data-v-2cba5b1a${_scopeId}>${ssrInterpolate(chatCustomer.value?.name || "Unknown")}</div><div class="chat-header__number" data-v-2cba5b1a${_scopeId}>${ssrInterpolate(chatCustomer.value?.contact)}</div></div><button class="chat-header__close" data-v-2cba5b1a${_scopeId}>×</button></div><div class="chat-container" data-v-2cba5b1a${_scopeId}><div class="chat-messages" data-v-2cba5b1a${_scopeId}>`);
						if (!whatsappConnected.value) _push(`<div class="chat-banner" data-v-2cba5b1a${_scopeId}> WhatsApp not connected — showing saved messages </div>`);
						else _push(`<!---->`);
						if (loadingMessages.value) _push(`<div class="chat-notice" data-v-2cba5b1a${_scopeId}>Loading messages…</div>`);
						else if (displayMessages.value.length === 0) _push(`<div class="chat-empty" data-v-2cba5b1a${_scopeId}>No messages yet</div>`);
						else {
							_push(`<!--[-->`);
							ssrRenderList(displayMessages.value, (msg) => {
								_push(`<div class="${ssrRenderClass([{ "chat-message--outgoing": msg.direction === "outgoing" }, "chat-message"])}" data-v-2cba5b1a${_scopeId}><div class="chat-message__bubble" data-v-2cba5b1a${_scopeId}>`);
								if (msg.media) {
									_push(`<!--[-->`);
									if (msg.media.kind === "image" || msg.media.kind === "sticker") _push(`<img${ssrRenderAttr("src", msg.media.url)} class="${ssrRenderClass([{ "chat-message__image--sticker": msg.media.kind === "sticker" }, "chat-message__image"])}" alt="attachment" data-v-2cba5b1a${_scopeId}>`);
									else if (msg.media.kind === "video") _push(`<video${ssrRenderAttr("src", msg.media.url)} class="chat-message__video" controls preload="metadata" data-v-2cba5b1a${_scopeId}></video>`);
									else if (msg.media.kind === "audio") _push(`<audio${ssrRenderAttr("src", msg.media.url)} class="chat-message__audio" controls preload="metadata" data-v-2cba5b1a${_scopeId}></audio>`);
									else _push(`<a${ssrRenderAttr("href", msg.media.url)}${ssrRenderAttr("download", msg.media.filename)} target="_blank" rel="noopener" class="chat-message__doc" data-v-2cba5b1a${_scopeId}><svg viewBox="0 0 24 24" width="26" height="26" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" data-v-2cba5b1a${_scopeId}><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" data-v-2cba5b1a${_scopeId}></path><polyline points="14 2 14 8 20 8" data-v-2cba5b1a${_scopeId}></polyline></svg><span class="chat-message__doc-name" data-v-2cba5b1a${_scopeId}>${ssrInterpolate(msg.media.filename)}</span></a>`);
									_push(`<!--]-->`);
								} else _push(`<!---->`);
								if (msg.text) _push(`<div class="chat-message__text" data-v-2cba5b1a${_scopeId}>${ssrInterpolate(msg.text)}</div>`);
								else _push(`<!---->`);
								_push(`<div class="chat-message__time" data-v-2cba5b1a${_scopeId}>${ssrInterpolate(formatMessageTime(msg.timestamp))}</div></div></div>`);
							});
							_push(`<!--]-->`);
						}
						_push(`</div>`);
						if (attachment.value) {
							_push(`<div class="chat-attachment" data-v-2cba5b1a${_scopeId}>`);
							if (attachmentIsImage.value) _push(`<img${ssrRenderAttr("src", attachmentPreview.value)} class="chat-attachment__thumb" alt="preview" data-v-2cba5b1a${_scopeId}>`);
							else _push(`<div class="chat-attachment__icon" data-v-2cba5b1a${_scopeId}><svg viewBox="0 0 24 24" width="32" height="32" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" data-v-2cba5b1a${_scopeId}><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" data-v-2cba5b1a${_scopeId}></path><polyline points="14 2 14 8 20 8" data-v-2cba5b1a${_scopeId}></polyline></svg></div>`);
							_push(`<span class="chat-attachment__name" data-v-2cba5b1a${_scopeId}>${ssrInterpolate(attachment.value.name)}</span><button class="chat-attachment__remove" data-v-2cba5b1a${_scopeId}>×</button></div>`);
						} else _push(`<!---->`);
						_push(`<div class="chat-input-row" data-v-2cba5b1a${_scopeId}><button class="chat-attach-btn" title="Attach file"${ssrIncludeBooleanAttr(!whatsappConnected.value || sendingMessage.value) ? " disabled" : ""} data-v-2cba5b1a${_scopeId}><svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" data-v-2cba5b1a${_scopeId}><path d="M21.44 11.05l-9.19 9.19a6 6 0 01-8.49-8.49l9.19-9.19a4 4 0 015.66 5.66l-9.2 9.19a2 2 0 01-2.83-2.83l8.49-8.48" data-v-2cba5b1a${_scopeId}></path></svg></button><input type="file" accept="image/*,video/*,audio/*,.pdf,.doc,.docx,.xls,.xlsx,.txt,.zip,.rar" class="chat-file-input" data-v-2cba5b1a${_scopeId}><input${ssrRenderAttr("value", messageInput.value)} type="text" class="chat-input" placeholder="Type a message"${ssrIncludeBooleanAttr(!whatsappConnected.value || sendingMessage.value) ? " disabled" : ""} data-v-2cba5b1a${_scopeId}><button class="chat-send-btn"${ssrIncludeBooleanAttr(!messageInput.value.trim() && !attachment.value || !whatsappConnected.value || sendingMessage.value) ? " disabled" : ""} title="Send" data-v-2cba5b1a${_scopeId}>`);
						if (!sendingMessage.value) _push(`<svg viewBox="0 0 24 24" width="22" height="22" fill="currentColor" data-v-2cba5b1a${_scopeId}><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z" data-v-2cba5b1a${_scopeId}></path></svg>`);
						else _push(`<span class="chat-send-spinner" data-v-2cba5b1a${_scopeId}></span>`);
			...
			_push(ssrRenderComponent(Modal_default, {
				...
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) _push(`<div class="call-card" data-v-2cba5b1a${_scopeId}><div class="call-avatar" data-v-2cba5b1a${_scopeId}>${ssrInterpolate((incomingCall.value?.name || incomingCall.value?.number || "?").charAt(0).toUpperCase())}</div><div class="call-name" data-v-2cba5b1a${_scopeId}>${ssrInterpolate(incomingCall.value?.name || "Unknown")}</div><div class="call-number" data-v-2cba5b1a${_scopeId}>${ssrInterpolate(incomingCall.value?.number)}</div><div class="call-status" data-v-2cba5b1a${_scopeId}>Incoming voice call…</div><button class="call-reject" title="Reject call" data-v-2cba5b1a${_scopeId}><svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor" data-v-2cba5b1a${_scopeId}><path d="M21 15.46l-5.27-.61-2.52 2.52a15.045 15.045 0 01-6.59-6.59l2.53-2.53L8.54 3H3.03C2.45 13.18 10.82 21.55 21 20.97v-5.51z" data-v-2cba5b1a${_scopeId}></path></svg></button></div>`);
			...
			_push(ssrRenderComponent(Modal_default, {
				...
				footer: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) _push(`<button class="ui-btn ui-btn--ghost" data-v-2cba5b1a${_scopeId}>Cancel</button><button class="ui-btn ui-btn--primary"${ssrIncludeBooleanAttr(normalizeNumber(composeNumber.value).length < 7) ? " disabled" : ""} data-v-2cba5b1a${_scopeId}> Start Chat </button>`);
					else return [createVNode("button", {
						class: "ui-btn ui-btn--ghost",
				...
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						...
						if (matchedCustomer.value) _push(`<p class="compose-match" data-v-2cba5b1a${_scopeId}> Matches customer: <strong data-v-2cba5b1a${_scopeId}>${ssrInterpolate(matchedCustomer.value.name)}</strong></p>`);
						else _push(`<!---->`);
			...
			_push(ssrRenderComponent(Modal_default, {
				...
				footer: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) _push(`<button class="ui-btn ui-btn--ghost" data-v-2cba5b1a${_scopeId}>Close</button>`);
					else return [createVNode("button", {
						class: "ui-btn ui-btn--ghost",
				...
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						if (ordersLoading.value) _push(`<p class="ord-hint" data-v-2cba5b1a${_scopeId}>Loading orders…</p>`);
						else {
							_push(`<!--[--><div class="ord-summary" data-v-2cba5b1a${_scopeId}><div data-v-2cba5b1a${_scopeId}><span data-v-2cba5b1a${_scopeId}>Total Orders</span><strong data-v-2cba5b1a${_scopeId}>${ssrInterpolate(customerOrders.value.length)}</strong></div><div data-v-2cba5b1a${_scopeId}><span data-v-2cba5b1a${_scopeId}>Total Spent</span><strong data-v-2cba5b1a${_scopeId}>${ssrInterpolate(money(customerOrdersTotal.value))}</strong></div><div data-v-2cba5b1a${_scopeId}><span data-v-2cba5b1a${_scopeId}>Contact</span><strong data-v-2cba5b1a${_scopeId}>${ssrInterpolate(ordersCustomer.value?.contact || "—")}</strong></div><div data-v-2cba5b1a${_scopeId}><span data-v-2cba5b1a${_scopeId}>Date of Birth</span><strong data-v-2cba5b1a${_scopeId}>${ssrInterpolate(formatDate(ordersCustomer.value?.date_of_birth))}</strong></div></div><div class="ord-detail" data-v-2cba5b1a${_scopeId}><span data-v-2cba5b1a${_scopeId}>Address</span><p data-v-2cba5b1a${_scopeId}>${ssrInterpolate(ordersCustomer.value?.address || "—")}</p></div>`);
							if (!customerOrders.value.length) _push(`<p class="ord-hint" data-v-2cba5b1a${_scopeId}>This customer has no orders yet.</p>`);
							else _push(`<!---->`);
							_push(`<!--[-->`);
							ssrRenderList(customerOrders.value, (o) => {
								_push(`<div class="ord-card" data-v-2cba5b1a${_scopeId}><div class="ord-card__head" data-v-2cba5b1a${_scopeId}><strong data-v-2cba5b1a${_scopeId}>Order #${ssrInterpolate(o.id)}</strong><span class="${ssrRenderClass([statusClass(o.status), "ui-badge"])}" data-v-2cba5b1a${_scopeId}>${ssrInterpolate(o.status || "—")}</span><span class="${ssrRenderClass([o.paid ? "ui-badge--success" : "ui-badge--muted", "ui-badge"])}" data-v-2cba5b1a${_scopeId}>${ssrInterpolate(o.paid ? "Paid" : "Unpaid")}</span><span class="ord-card__type" data-v-2cba5b1a${_scopeId}>${ssrInterpolate(typeLabel(o.type))}</span><span class="ord-card__date" data-v-2cba5b1a${_scopeId}>${ssrInterpolate(dateTime(o.order_datetime))}</span></div><table class="data-table ord-items" data-v-2cba5b1a${_scopeId}><thead data-v-2cba5b1a${_scopeId}><tr data-v-2cba5b1a${_scopeId}><th data-v-2cba5b1a${_scopeId}>Item</th><th style="${ssrRenderStyle({ "text-align": "center" })}" data-v-2cba5b1a${_scopeId}>Qty</th><th style="${ssrRenderStyle({ "text-align": "right" })}" data-v-2cba5b1a${_scopeId}>Subtotal</th></tr></thead><tbody data-v-2cba5b1a${_scopeId}>`);
								if (!orderLines(o).length) _push(`<tr data-v-2cba5b1a${_scopeId}><td colspan="3" class="data-table__empty" data-v-2cba5b1a${_scopeId}>No items on this order.</td></tr>`);
								else _push(`<!---->`);
								_push(`<!--[-->`);
								ssrRenderList(orderLines(o), (it) => {
									_push(`<tr data-v-2cba5b1a${_scopeId}><td data-v-2cba5b1a${_scopeId}>${ssrInterpolate(itemName(it.fooditems_id))} `);
									if (it.add_note) _push(`<small class="ord-note" data-v-2cba5b1a${_scopeId}>${ssrInterpolate(it.add_note)}</small>`);
									else _push(`<!---->`);
									_push(`</td><td style="${ssrRenderStyle({ "text-align": "center" })}" data-v-2cba5b1a${_scopeId}>${ssrInterpolate(it.quantity)}</td><td style="${ssrRenderStyle({ "text-align": "right" })}" data-v-2cba5b1a${_scopeId}>${ssrInterpolate(money(it.sub_total))}</td></tr>`);
								});
								_push(`<!--]--></tbody></table><div class="ord-card__foot" data-v-2cba5b1a${_scopeId}><span data-v-2cba5b1a${_scopeId}>Grand Total</span><strong data-v-2cba5b1a${_scopeId}>${ssrInterpolate(money(o.grand_total))}</strong></div></div>`);
							});
							_push(`<!--]--><!--]-->`);
						}
			...
			_push(ssrRenderComponent(Modal_default, {
				...
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						_push(`<p class="ord-summary ord-summary--calls" data-v-2cba5b1a${_scopeId}><span data-v-2cba5b1a${_scopeId}>${ssrInterpolate(customerCalls.value.length)} recent call${ssrInterpolate(customerCalls.value.length === 1 ? "" : "s")}</span></p>`);
						if (!customerCalls.value.length) _push(`<p class="ord-hint" data-v-2cba5b1a${_scopeId}>No recent calls for this number.</p>`);
						else {
							_push(`<ul class="call-list" data-v-2cba5b1a${_scopeId}><!--[-->`);
							ssrRenderList(customerCalls.value, (c, i) => {
								_push(`<li class="call-list__item" data-v-2cba5b1a${_scopeId}><span class="${ssrRenderClass([`call-list__icon--${callDirection(c)}`, "call-list__icon"])}" data-v-2cba5b1a${_scopeId}><svg viewBox="0 0 24 24" width="15" height="15" fill="currentColor" data-v-2cba5b1a${_scopeId}><path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z" data-v-2cba5b1a${_scopeId}></path></svg></span><span class="call-list__meta" data-v-2cba5b1a${_scopeId}><span class="call-list__dir" data-v-2cba5b1a${_scopeId}>${ssrInterpolate(callDirection(c) === "outgoing" ? "Outgoing" : "Incoming")} call</span><span class="call-list__time" data-v-2cba5b1a${_scopeId}>${ssrInterpolate(formatMessageTime(callTime(c)))}</span></span><span class="${ssrRenderClass([callStatusClass(c.status), "ui-badge"])}" data-v-2cba5b1a${_scopeId}>${ssrInterpolate(c.status || "—")}</span></li>`);
							});
							_push(`<!--]--></ul>`);
						}
			...
			if (lightboxUrl.value) _push(`<div class="chat-lightbox" data-v-2cba5b1a><img${ssrRenderAttr("src", lightboxUrl.value)} class="chat-lightbox__img" alt="attachment" data-v-2cba5b1a></div>`);
			else _push(`<!---->`);
			_push(`</div>`);
		};
	}
...
var Customers_default = /*#__PURE__*/ _plugin_vue_export_helper_default(_sfc_main, [["__scopeId", "data-v-2cba5b1a"]]);
//#endregion
